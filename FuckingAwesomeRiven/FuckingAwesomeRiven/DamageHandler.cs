    ...
    internal class DamageHandler
    {
        ...
        public static double getComboDmg(bool useR, Obj_AI_Hero target)
        {
            ...
            var bonusAD = ObjectManager.Player.FlatPhysicalDamageMod +
                          (useR && !CheckHandler.RState && SpellHandler._spells[SpellSlot.R].IsReady()
                              ? 0.2 *
            ...
            if (SpellHandler._spells[SpellSlot.Q].IsReady())
            {
                dmg += (3 - CheckHandler.QCount) *
                       (new double[] { 10, 30, 50, 70, 90 }[SpellHandler._spells[SpellSlot.Q].Level - 1] +
                        ((baseAD + bonusAD) / 100) *
            ...
            {
                dmg += (new double[] { 50, 80, 110, 140, 170 }[SpellHandler._spells[SpellSlot.W].Level - 1]) +
                       1 * bonusAD;
                passiveCount++;
            }
            ...
            if (useR && SpellHandler._spells[SpellSlot.R].IsReady())
            {
                ...
                dmg += new double[] { 80, 120, 160 }[SpellHandler._spells[SpellSlot.R].Level - 1] +
        ...
        {
            return new double[] { 10, 30, 50, 70, 90 }[SpellHandler._spells[SpellSlot.Q].Level] +
                   ((ObjectManager.Player.BaseAttackDamage + ObjectManager.Player.FlatPhysicalDamageMod +
                     (calcR ? rBonus : 0)) / 100) *
        ...
        {
            return new double[] { 50, 80, 110, 140, 170 }[SpellHandler._spells[SpellSlot.W].Level] +
        ...
        public static double rDamage(Obj_AI_Base target, int healthMod = 0)
        {
            var health = (target.MaxHealth - (target.Health - healthMod)) > 0
                ...
                : 1;
            return (new double[] { 80, 120, 160 }[SpellHandler._spells[SpellSlot.R].Level] +
