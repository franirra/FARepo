    ...
    class AxeCatcher
    {
        ...
        public static void catchAxes()
        {
            ...
            if (selectedAxe == null || AxeSpots.Count == 0 || Player.Distance(selectedAxe.AxeObj.Position) < 110 || GetTarget().IsValid<Obj_AI_Hero>() && Player.GetAutoAttackDamage(GetTarget() as Obj_AI_Base) * 2 > GetTarget().Health || !Program.Config.Item("catching").GetValue<KeyBind>().Active || !inCatchRadius(selectedAxe))
            {
                ...
                if (Orbwalking.CanMove(Program.Config.Item("ExtraWindup").GetValue<Slider>().Value) && Game.CursorPos.Distance(Player.Position) > Program.Config.Item("HoldPosRadius").GetValue<Slider>().Value && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.None)
                {
            ...
            else if (Orbwalking.CanMove(Program.Config.Item("ExtraWindup").GetValue<Slider>().Value))
            {
